                        ...
                        if (global.autoLike) {
                            try {
                                ...
                                if (now - lastStatusReactTime < (global.statusReactThrottleMs ?? 5000)) {
                                    ...
                                    // FIX: metadata-only status ping (message === null) — nothing
                                    // to react to. getContentType(null) would throw here, which
                                    // was silently caught below and made reacts look "random".
                                    console.log('⚠️ Skipped react — metadata-only status ping (no message payload)');
                                } else {
                                    ...
                                    // FIX: unwrap view-once statuses. Newer WhatsApp clients often
                                    // send image/video statuses wrapped in viewOnceMessageV2 /
                                    // viewOnceMessageV2Extension — checking the outer contentType
                                    // against the whitelist always failed for these, so real
                                    // photo/video statuses were silently skipped.
                                    let contentType = getContentType(rawMsg.message);
                                    if (contentType === 'viewOnceMessageV2' || contentType === 'viewOnceMessageV2Extension') {
                                        const inner = rawMsg.message.viewOnceMessageV2?.message || rawMsg.message.viewOnceMessageV2Extension?.message;
                                        if (inner) contentType = getContentType(inner);
                                    }
                                    ...
                                    if (reactable.includes(contentType)) {
                                        ...
                                        console.log(`✅ Status react: ${emoji} → ${realJid}`);
                                        lastStatusReactTime = Date.now();
                                        await new Promise(resolve => setTimeout(resolve, global.statusReactDelayMs ?? 2000));
                                    } else {
                                        console.log(`⚠️ Skipped react — unhandled status contentType: ${contentType}`);
                                    }
                                }
                            } catch (err) {
